...
import { LogoIcon, UserIcon, LockClosedIcon, EyeIcon, EyeOffIcon } from './components/icons';


// --- Auth Components moved here to free up LandingPage.tsx for the new hero page ---

// Login Screen Component (from original screens/LandingPage.tsx)
  ...
  return (
    <div className="min-h-screen bg-white flex font-sans animate-fade-in">
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-600 via-indigo-700 to-blue-800 relative p-12 text-white flex-col justify-between">
        <div className="absolute top-0 left-0 w-full h-full opacity-10" style={{backgroundImage: 'radial-gradient(#FFF 1px, transparent 1px)', backgroundSize: '2rem 2rem'}}></div>
        <div className="z-10">
          <h1 className="text-5xl font-bold tracking-tight">Welcome Back!</h1>
          <p className="mt-4 text-lg text-blue-100 opacity-80">Log in to continue your brain training journey and see your progress.</p>
        </div>
        <div className="z-10 flex gap-4">
            <button className="px-5 py-2 border-2 border-white rounded-md font-semibold hover:bg-white/10 transition-colors">Learn More</button>
            <button className="px-5 py-2 border-2 border-white rounded-md font-semibold hover:bg-white/10 transition-colors">Watch Demo</button>
        </div>
        <div className="z-10">
      ...
      <main className="w-full lg:w-1/2 p-8 sm:p-12 flex flex-col justify-center">
        <div className="max-w-md mx-auto w-full">
            ...
            <h2 className="text-3xl font-bold text-gray-900">Welcome to Company</h2>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                <div>
                    ...
                    <div className="mt-1 relative">
                        ...
                        <input type="email" id="email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-indigo-500"/>
                    </div>
                </div>
                <div>
                    ...
                    <div className="mt-1 relative">
                        ...
                        <input type={isPasswordVisible ? 'text' : 'password'} id="password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                ...
                <div className="flex items-center gap-4">
                    <button type="submit" disabled={isLoading} className="w-1/2 flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">
                        {isLoading ? 'Logging In...' : 'Login'}
                    </button>
                     <button type="button" onClick={onNavigateToSignup} className="w-1/2 text-center py-3 px-4 rounded-md text-lg font-medium text-indigo-600 hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Sign Up
                    </button>
                </div>
            </form>
...
// Signup Screen Component (from components/AuthModal.tsx)
interface SignupScreenProps {
  onSuccess: () => void;
  onNavigateToLogin: () => void;
}

const SignupScreen: React.FC<SignupScreenProps> = ({ onSuccess, onNavigateToLogin }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [isPasswordVisible, setIsPasswordVisible] = useState(false);
...
// --- Main App Component ---
type AppState = 'landing' | 'auth' | 'loggedIn';
...
const App: React.FC = () => {
  ...
  const handleNavigateToAuth = (page: 'login' | 'signup' = 'signup') => {
      setAuthPage(page);
      setAppState('auth');
  }
